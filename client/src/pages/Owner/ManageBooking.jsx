...
const ManageBooking = () => {
  ...
  return (
    <div className='px-4 pt-10 w-full md:px-10'>
      <Title title="Manage Bookings" subtitle='View all listed Vehicles ,and  Manage these '/>
                ...
                <td className='p-3'>
                    {
                      booking.status ==="pending" ?(
                        <select onChange={e=>changeBookingsStatus(booking._id,e.target.value)} value={booking.status} className='px-2 py-1.5 mt-1 text-gray-500 border border-borderColor rounded-md outline-none'>
                          <option value="pending">Pending</option>
                          <option value="cancelled">Cancelled</option>
                          <option value="confirmed">Confirmed</option>
                        </select>
                      ):(
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${booking.status ? 'bg-green-200 text-green-500':'bg-red-200 text-red-500'}`}>
                          {booking.status}
                        </span>
                      )
                    }
